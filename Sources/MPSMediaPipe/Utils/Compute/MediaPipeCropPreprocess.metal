...
// Inlined rather than shared via #include -- only this file and
// MediaPipeSegmentationMaskWarp.metal need it.
...
// For a centered output pixel offset (px, py) in source-rect-local pixels,
// the corresponding source-image offset is R(+rotationRadians) * (px, py)
// -- rotate the output-local offset directly, no negation.
struct MediaPipeCropUniforms {
    ...
    uint2 outputSize;
    float2 outputPixelRange;         // (min, max) the sampled [0,1] color maps onto
};
    ...
    uint2 position [[thread_position_in_grid]])
{
    ...
    // NHWC (model input is [1, H, W, 3], not NCHW).
